    ...
    >
      <form
        ...
        onSubmit={async (e) => {
          ...
          setError("");
          try {
            const response = await fetch("https://formspree.io/f/mrpzjrng", {
              method: "POST",
      ...
      >
        {[["Name", "name"], ["Company", "company"], ["Work email", "email"], ["Phone", "phone"]].map(([x, fieldName]) => (
          <label key={x}>
            {x}
            <input required name={fieldName} type={fieldName === "email" ? "email" : "text"} placeholder={`Enter ${x.toLowerCase()}`} />
          </label>
        ))}
        <label>
          Company size
        ...
        <label>
          Message
          <textarea name="message" rows="5" placeholder="What are you trying to detect?" />
        </label>
        {error && <div className="form-error" role="alert">{error}</div>}
        <Button icon={sent ? Check : ArrowRight} disabled={submitting}>
          {submitting ? "Sending request..." : sent ? "Request received" : "Request a demo"}
        </Button>
      </form>
    </PublicPage>
  );
  ...
  return (
    ...
    >
      <div className="kpi-grid">
        ...
        <KPI label="Critical alerts" value="184" detail="21 need attention" tone="red" />
        <KPI label="Open cases" value={327 + cases.length} detail="8 due today" tone="orange" />
        <KPI label="Fraud prevented" value="₹4.82 Cr" detail="+18.4% this month" tone="green" />
        <KPI label="Detection accuracy" value="96.4%" detail="Across 6 engines" tone="violet" />
      </div>
        ...
        <div className="panel">
          ...
          <ResponsiveContainer width="100%" height={245}>
            <AreaChart data={chart}>
              ...
              <CartesianGrid stroke="#242a34" vertical={false} />
              <XAxis
                dataKey="n"
                stroke="#6b7484"
                tickLine={false}
                axisLine={false}
              />
              <YAxis stroke="#6b7484" tickLine={false} axisLine={false} />
              <Tooltip
                contentStyle={{
                  background: "#161b23",
...
export function Detection({ type, guest = false }) {
  ...
  const runAnalysis = async () => {
    ...
    setError("");
    try {
      const result = await api.analyze({ type, source, context });
  ...
  if (guest) {
    return (
      <PublicPage
        ...
        copy={m.description}
      >
        <div className="panel guest-module-panel">
          <div className="module-icon" style={{ "--module": m.color }}>
            <m.icon size={28} />
          </div>
          <span className="eyebrow">GUEST MODE</span>
          <h2>Explore this detection engine</h2>
          <p>
            Review how {m.name.toLowerCase()} intelligence fits into the
            Cybershield workflow. Sign in to submit a signal and run AI analysis.
          </p>
          <Link className="button button-primary" to="/login">
            Sign in to analyze <ArrowRight size={15} />
          </Link>
        </div>
      </PublicPage>
    );
    ...
    >
      <div className="workbench">
        <div className="panel form-panel">
          ...
          {error && <div className="form-error" role="alert">{error}</div>}
          <Button icon={Zap} onClick={runAnalysis} disabled={loading || (!source && !context)}>
            {loading ? "Analyzing..." : done ? "Analysis complete" : "Analyze signal"}
          </Button>
        </div>
          ...
          {done ? (
            <>
              <div className="result-score">
                <strong>{analysis?.riskScore ?? m.score}</strong>
                <span>/100</span>
                <Badge tone={(analysis?.riskLevel || (m.score > 92 ? "critical" : "high")).toLowerCase()}>
                  {analysis?.riskLevel || (m.score > 92 ? "Critical" : "High")}
                </Badge>
              </div>
              <div className="factors">
                {(analysis?.reason ? [analysis.reason] : [
                  ...
                  <div key={x}>
                    ...
                    <b>
                      <i
                        style={{
                          width: `${analysis?.riskScore ?? m.score - i * 8}%`,
                          background: m.color,
                        }}
                      />
                    </b>
              ...
              <div className="analysis-note">
                <Target size={17} />
                <span>
                  <strong>Explainable recommendation</strong>{analysis?.recommendation || "Quarantine signal and initiate investigation."}
                </span>
              </div>
            </>
          ) : (
