...
export function AppRouter() {
  ...
  const guestModule = !user && path.startsWith("/detections/");
  const appPage = getAppPage(path, { guest: guestModule });
  ...
  if (!user && isProtectedPath(path) && !guestModule)
    return (
      <ErrorPage
        code="SESSION"
  ...
  return guestModule ? appPage : <Shell>{appPage}</Shell>;
}
